...
class VirtualPULSEModel < OpenStudio::Model::Model
 ...
 ######################################################## 
  def add_space_type(params)
    ...
    remoteBCL.setProdAuthKey("xsxYuim9hvuuGdVFvhM5GBxNLPnDmNgE") 

    #remoteBCL.downloadOnDemandGenerator("bb8aa6a0-6a25-012f-9521-00ff10704b07");
    remoteBCL.downloadOnDemandGenerator("c5c74454-7146-4212-9569-41791ec47defI");    
 ...
 ######################################################## 
  def add_design_days(params)
	...
	#load the design days for Chicago
        ...
        # notice: ddy_path is changed to idf file instead of ddy file
        ddy_path = OpenStudio::Path.new("#{weather_path.to_s}/#{location.to_s}.idf")    
	#make sure the file exists on the filesystem; if it does, open it
	if OpenStudio::exists(ddy_path)
          
          # load function does not support ddy extension file?
 ...
 ############################################################## 
  def self.run_energyplus_simulation(params)
    ...
    require 'openstudio/energyplus/find_energyplus'
    puts "<br>##################### debugging run_energyplus_simulation ################<br>"
    ...
    simulation_time = Time.now.to_i
    run_time = simulation_time - time0
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the find energyplus time is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"
    ...
    epw_path = OpenStudio::Path.new("#{weather_path.to_s}/#{location}.epw")
    ...
    simulation_time = Time.now.to_i
    run_time = simulation_time - time1
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the run manager time is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"
    ...
    simulation_time = Time.now.to_i
    run_time = simulation_time - time2
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the setup tool info to pass run manager the location of energy plus time is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"
    ...
    simulation_time = Time.now.to_i
    run_time = simulation_time - time3
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the get the run manager configuration options time is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"
    ...
    simulation_time = Time.now.to_i
    run_time = simulation_time - time4
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the ENERGYPLUS directory to store the results time is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"
    ...
    job = OpenStudio::Runmanager::JobFactory::createEnergyPlusJob(ep_tool,
                                                                 ...
                                                                 output_path)
                                                                    ...
                                                                    simulation_time = Time.now.to_i
    run_time = simulation_time - time5
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the job for the file we want to run time is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"
    ...
    simulation_time = Time.now.to_i
    run_time = simulation_time - time6
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the put the job in the run queue time is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"
    
    # This takes most of time
    time7 = Time.now.to_i
    # wait for jobs to complete
    while run_manager.workPending()
      check=OpenStudio::Application::instance().processEvents()
    end
    
    simulation_time = Time.now.to_i
    run_time = simulation_time - time7
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the total time on jobs is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"

    simulation_time = Time.now.to_i
    run_time = simulation_time - start_time
    run_time_in_mins = 1.0*run_time / 60.0 
    puts "###the total simulation time is " + run_time.to_s + "seconds, " + run_time_in_mins.to_s + " mins####"
    
    puts "finished running #{idf_name}"
    puts "<br>################################# End debugging ###################<br>"
  end  

...
if (!isset($_SESSION['db_is_logged_in'])
    ...
    $connect = mssql_connect("$hostName", "$sqlusername", "$sqlpassword")
	or die("ERROR: selecting database server failed");

    // Select database
    mssql_select_db($databaseName, $connect)
    ...
    // Execute query
    $query_result = mssql_query( $query, $connect)
    ...
    // Get field names
    echo "<tr>\n"; // Start a new row
    while ($field = mssql_fetch_field($query_result)) { 

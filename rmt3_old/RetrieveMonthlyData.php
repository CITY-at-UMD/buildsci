...
*/
function retriveMonthlyData($path,$htmlFile,$selectColumn, $totalColumn) {
      ...
      // found the zone field
			...
			if ($isTgTable == 1) {
				...
				if(($curCol%$totalColumn) == $selectColumn) {
					...
					//echo $newValue[$key.$zone.$selectedAttribute];
					/*echo 'Zone '.$zone;
					echo ' '.$key.'(row '.($curRow%17).', column '.$selectColumn.'): '.$newValue[$key];
				  //echo 'at Table (row '.($curRow%17).', column '.($selectColumn%17).')';*/
				}
			}
		}
...
$data= retriveMonthlyData("./","eplustbl.htm",3,5);
